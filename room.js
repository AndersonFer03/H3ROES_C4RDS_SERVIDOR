const { buildUnique } = require("./utils");
...
function findAvailableRoom(preferRoomId) {
  if (preferRoomId && rooms[preferRoomId]) {
    ...
    if ((r.players.p1 ? 1 : 0) + (r.players.p2 ? 1 : 0) < 2) return preferRoomId;
  }
  for (const [id, r] of Object.entries(rooms)) {
    if ((r.players.p1 ? 1 : 0) + (r.players.p2 ? 1 : 0) < 2) return id;
  }
  return createRoom();
}
...
module.exports = { createRoom, findAvailableRoom, broadcast, playersCount, bothPlayersReady, resetRound, rooms };
